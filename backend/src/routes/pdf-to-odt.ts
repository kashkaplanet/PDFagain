...
export const postHandler = async (req: Request, res: Response) => {
    ...
    try {
        ...
        inputPath = path.join(os.tmpdir(), `${timestamp}_${filename}.pdf`);
        outputPath = path.join(os.tmpdir(), `${timestamp}_${filename}.odt`);
        ...
        // Call Python script
        const scriptPath = path.join(process.cwd(), 'scripts', 'convert_pdf_to_odt.py');
        ...
        const odtBuffer = fs.readFileSync(outputPath);
        ...
        res.set({
            ...
            "Content-Length": odtBuffer.length.toString(),
        });
        return res.send(odtBuffer);
        ...
        return handleApiError(res, error, "Internal server error during ODT conversion");
    }
}
