...
export const postHandler = async (req: Request, res: Response) => {
    try {
        ...
        if (pdfBuffer.length < 4 || pdfBuffer.toString('utf-8', 0, 4) !== '%PDF') {
            return handleBadRequest(res, "Invalid binary file. The decoded data is not a valid PDF.");
        }

        const outputFilename = file.originalname.replace(/\.bin\.txt$/i, "").replace(/\.[^/.]+$/, "") + ".pdf";
        ...
        return handleApiError(res, error, "Internal server error during binary to PDF conversion");
    }
}
