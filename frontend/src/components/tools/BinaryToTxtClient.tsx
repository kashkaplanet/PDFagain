...
export default function BinaryToTxtClient() {
    ...
    const handleConvert = async () => {
        ...
        try {
            const response = await fetch("/api/binary-to-txt", {
                method: "POST",
                body: formData,
            });
    ...
    const handleDownload = () => {
        if (convertedUrl && file) {
            ...
            a.href = convertedUrl;
            a.download = file.name.replace(/\.bin\.txt$/i, "").replace(/\.[^/.]+$/, "") + ".txt";
    ...
    return (
        <ToolPageWrapper
            ...
            description="Restore a text file from a Base64-encoded binary text file. Get back your original text file exactly as it was."
            icon={Shield}
            color="indigo"
        >
                ...
                <div className="mb-8">
                    {!file ? (
                        <RetroFileUploader
                            ...
                            description="Select a .bin.txt file to restore back to text"
                            variant="indigo"
                        />
                    ) : (
                        <RetroFileItem
