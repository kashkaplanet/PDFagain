...
export default function PdfToRtfClient() {
    ...
    const handleConvert = async () => {
        ...
        try {
            const response = await fetch("/api/pdf-to-rtf", {
                method: "POST",
                body: formData,
            });
    ...
    const handleDownload = () => {
        if (convertedUrl && file) {
            ...
            a.href = convertedUrl;
            a.download = file.name.replace(/\.[^/.]+$/, "") + ".rtf";
    ...
    return (
        <ToolPageWrapper
            title="PDF to RTF"
            description="Convert PDF documents to Rich Text Format files."
            icon={FileText}
            color="blue"
        >
