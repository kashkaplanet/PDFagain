...
export default function TxtToBinaryClient() {
    ...
    const handleConvert = async () => {
        ...
        try {
            const response = await fetch("/api/txt-to-binary", {
                method: "POST",
                body: formData,
            });
    ...
    return (
        <ToolPageWrapper
            ...
            description="Convert text files into Base64-encoded binary text files for safe storage and transfer. The original file can be perfectly restored."
            icon={Shield}
            color="indigo"
        >
                ...
                <div className="mb-8">
                    {!file ? (
                        <RetroFileUploader
                            ...
                            description="Select a .txt file to convert to binary text"
                            variant="indigo"
                        />
                    ) : (
                        <RetroFileItem
