...
export async function POST(req: NextRequest) {
    try {
        ...
        const scriptPath = join(process.cwd(), "scripts", "convert_csv_to_excel.py");
        ...
        return NextResponse.json(
            { error: "Conversion failed. Ensure Python and openpyxl are installed (pip install openpyxl)." },
            { status: 500 }
        );
    }
}
