...
export async function POST(req: NextRequest) {
    try {
        ...
        // Call Python script
        const scriptPath = join(process.cwd(), "scripts", "convert_word.py");
        ...
        // Cleanup
        // We clean up the whole request ID dir
        await Promise.all([
            unlink(inputPath).catch(() => { }),
            unlink(outputPath).catch(() => { })
        ]);
        // Try to remove dir if empty, but don't fail if not
        // fs.rm(tempDir, { recursive: true }) is better if node version supports it (Node 14+)
