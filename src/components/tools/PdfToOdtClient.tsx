...
export default function PdfToOdtClient() {
    ...
    const handleConvert = async () => {
        ...
        try {
            const response = await fetch("/api/pdf-to-odt", {
                method: "POST",
                body: formData,
            });
    ...
    const handleDownload = () => {
        if (convertedUrl && file) {
            ...
            a.href = convertedUrl;
            a.download = file.name.replace(/\.[^/.]+$/, "") + ".odt";
    ...
    return (
        <ToolPageWrapper
            title="PDF to ODT"
            description="Convert PDF documents to OpenDocument Text format."
            icon={FileText}
            color="blue"
        >
