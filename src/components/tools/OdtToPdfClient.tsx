...
export default function OdtToPdfClient() {
    ...
    const handleConvert = async () => {
        ...
        try {
            const response = await fetch("/api/convert/odt", {
                method: "POST",
                body: formData,
            });
    ...
    return (
        <ToolPageWrapper
            title="ODT to PDF"
            description="Convert OpenDocument Text files to PDF using LibreOffice/Word automation."
            icon={FileText}
            color="cyan"
        >
                ...
                <div className="mb-8">
                    {!file ? (
                        <RetroFileUploader
                            ...
                            accept={{ "application/vnd.oasis.opendocument.text": [".odt"] }}
                            multiple={false}
                            title="Upload ODT Document"
                            description="Select a .odt file to convert"
                            variant="cyan"
                        />
                    ) : (
                        <RetroFileItem
